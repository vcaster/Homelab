resource "vsphere_virtual_machine" "kube-test" {
  count = length(var.kubernetes-test)
  name             = var.kubernetes-test[count.index].name
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  datastore_id     = data.vsphere_datastore.datastore.id
  folder               = "Kubernetes-test"

  num_cpus             = var.kubernetes-test[count.index].cpus
  num_cores_per_socket = var.kubernetes-test[count.index].cores_per_socket
  memory               = var.kubernetes-test[count.index].ram
  ...
  disk {
    label            = "${var.kubernetes-test[count.index].name}-disk"
    ...
    size             = var.kubernetes-test[count.index].disk_size
  }
  ...
  extra_config = {
    "guestinfo.metadata"          = base64encode(templatefile("${path.module}/templates/ubuntu2204-server-metadata.yaml", {
        name         = var.kubernetes-test[count.index].name,
        ipv4_address = var.kubernetes-test[count.index].ip_addr,
