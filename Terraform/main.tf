...
locals {
  ...
  pi-hole-2-templatevars = {
    name         = var.pi-hole-2,
    ipv4_address = var.pi-hole-2-ipv4_address,
    ipv4_gateway = var.ipv4_gateway,
    dns_server_1 = var.dns_server_list[0],
    dns_server_2 = var.dns_server_list[1],
    public_key = var.public_key,
...
resource "vsphere_virtual_machine" "pi-hole-2" {
  name             = var.pi-hole-2
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  datastore_id     = data.vsphere_datastore.datastore.id
  folder               = "Github"
  ...
  extra_config = {
    "guestinfo.metadata"          = base64encode(templatefile("${path.module}/templates/ubuntu2204-server-metadata.yaml", local.pi-hole-2-templatevars))
    "guestinfo.metadata.encoding" = "base64"
    "guestinfo.userdata"          = base64encode(templatefile("${path.module}/templates/ubuntu2204-server-userdata.yaml", local.pi-hole-2-templatevars))
    "guestinfo.userdata.encoding" = "base64"
  }
  lifecycle {
    ignore_changes = [
